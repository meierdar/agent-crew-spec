...
4. `## A2A` section in stories (auto-generated by `ai-prep`)
5. `ai-prep` script: generates A2A from prose + bus
6. `critical: true` story flag for selective review/redundancy
7. "Not in scope" as mandatory story section (prevents gold-plating)
8. `ai-swarm plan` — visualize execution waves
9. `ai-ready-queue` — auto-discover next stories to assign
10. `ai-claim` — prevent double-assignment in parallel execution
11. State machine engine for `ai-backlog` transition validation
12. Signal files for ephemeral coordination events
13. Updated CLAUDE.md protocol: "prioritize ## A2A, read bus, write to bus"
